...
Runs INSIDE Codex. The `/relay` skill (`.Codex/skills/relay/SKILL.md`) is the
runtime — Codex reads Slack/Gmail via MCP, analyzes intent, and executes approved
...
relay/sources/   MessageSource contract + normalize() pure fns. Sources ORIGINATE
                 ...
                 Phase 2 adds multi-account via Codex API mcp_servers). Jira/Notion are
...
.Codex/skills/relay/             the scan + review orchestration skill
.Codex/skills/persona-bootstrap/ Phase A one-time batch build (NEVER in the loop)
```
...
- Every human-facing draft (reply/relay/forward) MUST pass the `leo-anti-ai-response`
  skill before sending — it layers Leo's actual voice (learned from his top-contact DMs)
...
- Phase 2 (eng-reviewed 2026-06-12; specs/phase2-cockpit-design.md). PRODUCT UNIT =
  ...
  only (approve/edit/skip via core; Codex stays the MCP runtime, flushes sends).
  DEFERRED to Phase 3: standalone Codex-API runtime, multi-account mcp_servers,
