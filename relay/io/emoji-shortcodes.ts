// Slack shortcode → Unicode emoji. GENERATED — do not hand-edit.
//
// Slack sends ':rolling_on_the_floor_laughing:' on the wire, not 🤣, so a
// transcript printed verbatim shows the code and not the reaction. Every name
// here is a STANDARD one; a workspace's custom emoji (:taiv-logo:) has no
// Unicode equivalent and is deliberately left as-is.
//
// Source: iamcal/emoji-data, which is the table Slack's own picker uses — the
// gemoji/GitHub names differ on exactly the cases that matter here (it calls
// that one ':rofl:'). Regenerate with:
//
//   curl -sL https://unpkg.com/emoji-datasource@16.0.0/emoji.json | node -e '…'
//
// (1972 names, Unicode 16.)
